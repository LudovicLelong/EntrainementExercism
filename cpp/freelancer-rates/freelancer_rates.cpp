...
double daily_rate(double hourly_rate) {
    ...
    // rate
    double hourOfWork = 8;
    return (hourly_rate * hourOfWork);
}

// apply_discount calculates the price after a discount
double apply_discount(double before_discount, double discount) {
    // TODO: Implement a function to calculate the price after a discount.
    double pourcentPlus = before_discount * (discount/ 100);
    double result = before_discount - pourcentPlus;
    return result;
}
...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    double hourDay = hourly_rate * 8;
    double montFay = hourDay * 22;
    double interPourc = montFay * (discount / 100);
    double rateCompt = montFay - interPourc;
    int result = ceil(rateCompt);
    return result;
}
